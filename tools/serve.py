...
"""Serve the record browser and rebuild it as the record grows.
    ...
    python serve.py ../demo_record               # http://localhost:8760
...
def rebuild(record_dir, out_dir, title):
    ...
    try:
        manifest = browse_v6.compile_record(record_dir, out_dir, title=title, quiet=True)
        with _LOCK:
            STATE["build"] += 1
            STATE["error"] = None
...
def main(argv=None):
    ...
    ap.add_argument("--out", default="dist_v6")
