#!/usr/bin/env python3
"""Compile a Symposium v6 CommunityRecord into a static, navigable browser.

    python browse_v6.py ../demo_record --out dist_v6

Written against v6 rather than migrated from the v2 compiler. The v2 compiler's model
layer solved a problem v6 does not have — v2 interposed a Bearing between an Assertion
and its evidence, and let edges carry cross-artifact addresses — so only its
presentation was worth keeping. That lives in `templates_v6.py`; this file produces the
element sets those templates consume.

Two facts about v6 shape everything here:
  ...
  * **All relationships are internal to one Artifact.** Every cross-artifact edge on
    ...
    `address`, a provenance field, or a markdown link in prose. There is no such thing
...
unverifiable references and bare prose citations are all things `validate_v6` already
...
from validate_v6 import (CITATION_RE, build_index, parse_address,     # noqa: E402
                         parse_instant, resolve, validate)
import templates_v6 as T                                              # noqa: E402

ARGUMENT = "Argument"
NON_GROUNDABLE_TYPES = {"Analysis", "Report", "Message"}
...
def load_record(record_dir):
    ...
    dotfiles, and `Path.glob('*.json')` matches those too. The v2 compiler globbed
    blindly and a member agent hit the crash during a live test.
    """
    arts = []
...
def cited_addresses(doc):
    """Addresses cited in prose, in the profile's markdown-link form (CANONICAL-v6 3.1).
    ...
    for d in scopes:
        for k, v in d.items():
            if not isinstance(v, str) or k in ("address", "published_by", "produced_by",
                                               "extracted_from"):
                continue
...
def build_claim_graph(doc, index, findings, colors, pages):
    ...
    assessments = {n: o for n, o in objs.items() if o.get("type") == "Assessment"}
    ...
    for an, a in assertions.items():
        my = out.get(an, [])
        asmts = [assessments[t] for rel, t in my if rel == "assessed_by" and t in assessments]
        verds = [x.get("verdict") for x in asmts if x.get("verdict")]
        claim = a.get("claim", "")
        ...
        alts = [{"id": t, "claim": assertions[t].get("claim")}
                for rel, t in my if rel == "has_alternative" and t in assertions]

        data = {
            "id": an, "ntype": "Assertion", "label": label,
            "owner": owner, "owner_color": color,
            "verdict": verds[0] if len(set(verds)) == 1 and verds else (verds[0] if verds else ""),
            "verdict_mixed": len(set(verds)) > 1,
            "n_assessments": len(asmts),
            "assessments": [dict(x) for x in asmts],
            "groundings": [{k: v for k, v in g.items() if not k.startswith("_")} for g in gs],
            "assumptions": us, "alternatives": alts,
            ...
            "tooltip": (("PRIMARY — " if an == h.get("primary_assertion") else "")
                        + (verds[0] + " — " if verds else "") + claim),
        }
        nodes.append({"data": data})
        full_nodes.append({"data": {k: v for k, v in data.items()
                                    if k not in ("assessments", "groundings", "assumptions")}})
        ...
        for rel, t in my:
            if rel in ("depends_on", "has_alternative") and t in assertions:
        ...
        for x in asmts:
            full_edges.append({"data": {"id": f"f_{an}_{x['name']}", "source": an,
                                        "target": x["name"], "rel": "assessed_by"}})
        for g in gs:
    ...
    # ---- assessments + grounds exist only in the full graph ------------------
    for xn, x in assessments.items():
        full_nodes.append({"data": {
            "id": xn, "ntype": "Assessment", "label": x.get("verdict", "?"),
            "owner": owner, "owner_color": color,
            "full": {k: v for k, v in x.items() if k != "name"},
            "tooltip": (x.get("verdict", "") + " — " + (x.get("purpose") or ""))[:200]}})
    ...
    meta = {
        ...
        "counts": {
            ...
            "folded_assessments": len(assessments), "folded_grounds": len(grounds),
...
def _layout_claim(nodes, edges, primary):
    ...
    for e in edges:
        if e["data"]["rel"] in ("depends_on", "has_alternative"):
...
_FULL_ORDER = {"Source": 0, "External": 0, "Ground": 1, "Assumption": 1,
               "Assertion": 2, "Assessment": 3}


def _layout_full(nodes, edges):
    """Layered by Object type: addressed content, then the Objects that reach it, then
    the Assertions, then the verdicts. The layering IS the v6 model, four columns wide
    where v2 needed five."""
    by_id = {n["data"]["id"]: n["data"] for n in nodes}
    col = defaultdict(list)
    for i, d in by_id.items():
...
_SHAPE = {"Argument": "round-rectangle", "Report": "hexagon", "Data": "rectangle",
        ...
        if a["artifact"]["type"] == ARGUMENT:
            ...
            if not quiet:
                ...
                print(f"  Argument  {name:44s} {c['nodes_total']:3d} nodes / "
                      ...
                      f"{c['verdicts']['insufficient']} insufficient"
...
def main(argv=None):
    ...
    ap.add_argument("record_dir", nargs="?",
                    default=os.environ.get("SYMPOSIUM_MIRROR", "../demo_record"),
                    help="directory of v6 canonical JSON (default: $SYMPOSIUM_MIRROR)")
    ap.add_argument("--out", default="dist_v6", help="output directory (default: dist_v6)")
