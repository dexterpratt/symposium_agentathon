"""Page templates for the Symposium v6 browser.

Carried over from the v2 compiler (`compile_review.py`) with the model vocabulary
rewritten for v6. The presentation — layout, interaction, detail panel, legend
mechanics — is the part worth keeping; what changed is what it names.

Substantive changes from the v2 originals:
  * the Bearing layer is gone. v6 puts a Ground directly on an Assertion, so the
    "full graph" is three layers, not four, and far less is folded away.
  * `requires`/`accepts` became `depends_on`/`has_alternative`; `assesses` became
    `assessed_by`; `grounds` became `grounded_by`.
  * grounding kind is no longer an enum: a Ground either carries a `criterion`
    (a test) or does not (evidential). The renderer keys off its presence.
  * Assumption is a node type of its own — v6 makes it a real Object, and it is
    exactly the weak joint a reader is looking for, so it is never folded away.
  * verdicts use underscores (`supported_for_purpose`).
  * `reviews` edges are gone. Cross-artifact citation is a markdown link in prose,
    so the markdown-lite renderer must PERMIT the one link form it used to forbid.
  * validator findings (independence, unverifiable grounding, bare citations) are
    rendered on the page rather than recomputed.
"""
...
<style>
  :root {{
    ...
    /* verdict encoding (folded from the collapsed Assessment) */
    --v-supported: #16a34a; --v-insufficient: #d97706; --v-falsified: #dc2626;
    --v-unknown: #9ca3af;
  }}
  * {{ box-sizing: border-box; }}
  ...
  /* claim-then-assessment sections in the detail panel */
  ...
  /* The Argument's own framing prose. It carries the author's citations, and a
     citation is the one place v6 puts nuance in prose rather than in vocabulary —
...
<div id="app">
  <div id="graphwrap">
    <div class="toolbar">
      ...
      <span class="flowkey" id="fullkey" style="display:none">every Object as authored: addressed content <b>&larr;</b> Ground &middot; Assumption &middot; Assessment &middot; Assertion <b>&rarr;</b></span>
    </div>
    <div id="cy"></div>
  ...
  <div id="side">
    <div id="legend"></div>
    <div id="detail"><p class="placeholder">Click an <b>assertion</b> for its claim, its assessment, and everything it rests on. Click an <b>assumption</b> for what is being granted and why. Click a <b>grounding</b> edge for its rationale and, where the author claimed a test, the criterion that could have refuted it.</p></div>
  </div>
</div>
...
(function () {{
  ...
  var NODE_COLORS = {{
    ...
    Assessment: '#7c3aed', Ground: '#0d9488', Assumption: '#d97706', External: '#9ca3af'
  }};
  var VERDICT_COLORS = {{
    'supported_for_purpose': '#16a34a', 'insufficient': '#d97706',
    'falsified': '#dc2626'
  }};
  ...
  //
  // "Full graph": every Object as authored (Assertion / Assessment / Ground / Assumption
  // + addressed content), for a reader who wants the uncollapsed structure. v6 folds far
  // less than v2 did — only the Assessment and the Ground hop — because there is no
  // Bearing layer to hide. Its own element set + layered layout (data._pos_full).
  //
  ...
  var cy = cytoscape({{
    ...
    style: [
      ...
      // Assertion = the collapsed claim+assessment node. It is the PRIMARY backbone,
      ...
      // verdict encoding (subtle: a colored border ring; polarity stays in the panel)
      {{ selector: 'node[ntype="Assertion"][verdict="supported_for_purpose"]', style: {{
          'border-color': '#16a34a' }} }},
      {{ selector: 'node[ntype="Assertion"][verdict="insufficient"]', style: {{
          'border-color': '#d97706' }} }},
      {{ selector: 'node[ntype="Assertion"][verdict="falsified"]', style: {{
          'border-color': '#dc2626' }} }},
      // >=2 coexisting assessments (future multi-author): dashed border + N badge
      {{ selector: 'node[ntype="Assertion"][?verdict_mixed]', style: {{
          'border-style': 'double' }} }},
      {{ selector: 'node[ntype="Assertion"][n_assessments > 1]', style: {{
          'border-style': 'dashed' }} }},
      ...
      {{ selector: 'node[ntype="Assumption"]', style: {{
          ...
          'text-max-width': 130 }} }},
      // (Full-graph mode) Ground + Assessment get their own encodings.
      ...
      {{ selector: 'node[ntype="Assessment"]', style: {{
          'shape': 'diamond', 'background-color': '#7c3aed', 'width': 22, 'height': 22,
      ...
      {{ selector: 'edge[rel="assessed_by"]', style: {{
          'line-color': '#7c3aed', 'target-arrow-color': '#7c3aed', 'width': 2 }} }},
      ...
      // `has_alternative` (assertion -> assertion): a rival reading the author is
      // naming rather than suppressing. Indigo, dashed, chevron — it leans, not needs.
      {{ selector: 'edge[rel="has_alternative"]', style: {{
          'line-color': '#4338ca', 'target-arrow-color': '#4338ca', 'width': 2.2,
          'line-style': 'dashed', 'target-arrow-shape': 'chevron', 'arrow-scale': 1.1 }} }},
      ...
      // Grounding, split by the ONE thing v6 records structurally about it: whether the
  ...
  function mdInline(escaped) {{
    ...
    // v2 forbade links outright. v6 makes a prose citation an address inside a
    // markdown link, which is what keeps a record navigable rather than merely
    // stored — so exactly [text](@address) and [text](<@address with spaces>) are
  ...
  function verdictColor(v) {{ return VERDICT_COLORS[v] || '#9ca3af'; }}

  // Detail for a COLLAPSED claim node: claim on top, its assessment(s) below.
  function renderClaim(data) {{
    ...
    html += '</dl></div>';
    // --- assessment section(s) (below) ---
    html += '<div class="section"><div class="sechd">'
         + (asmts.length === 1 ? 'Assessment'
            : asmts.length === 0 ? 'Assessment (none)'
            : asmts.length + ' Assessments') + '</div>';
    if (asmts.length === 0) {{
      html += '<p class="hint">No assessment folded into this claim.</p>';
    }}
    asmts.forEach(function (a) {{
      html += '<div class="asmt-card">';
      var v = a.verdict || '?';
      html += '<span class="verdict" style="background:' + verdictColor(v) + '">' + esc(v) + '</span>';
      html += '<dl>';
      ['purpose','evaluation'].forEach(function (k) {{
        if (k in a && a[k] != null && a[k] !== '')
          html += '<dt>' + esc(k) + '</dt><dd>' + fmt(a[k]) + '</dd>';
      }});
      // any other assessment fields, generically (skip id/type/verdict already shown)
      Object.keys(a).forEach(function (k) {{
        if (['id','name','type','verdict','purpose','evaluation'].indexOf(k) < 0)
          html += '<dt>' + esc(k) + '</dt><dd>' + fmt(a[k]) + '</dd>';
      }});
      html += '</dl></div>';
    }});
  ...
  // Detail for a has_alternative edge: a rival reading the author named rather than
  // suppressed. v6 records that an alternative EXISTS; whether it was excluded is a
  // matter for the Assessment, so this panel deliberately does not adjudicate.
  function renderAltEdge(data) {{
    var html = '<h2>Alternative</h2>';
    html += '<span class="pill" style="background:#4338ca">has_alternative</span>';
    html += '<div class="section"><dl>';
    html += '<dt>this assertion</dt><dd>' + esc(claimLabelFor(data.source)) + '</dd>';
    html += '<dt>has the rival reading</dt><dd><b>' + esc(claimLabelFor(data.target)) + '</b></dd>';
    html += '<dt></dt><dd class="hint">Naming an alternative does not exclude it. Whether the '
          + 'record distinguishes the two is stated in the assessment.</dd>';
    html += '</dl></div>';
    detail.innerHTML = html;
  }}
  ...
  function renderDetail(kind, data) {{
    ...
    if (kind === 'edge' && data.rel === 'has_alternative') {{ renderAltEdge(data); return; }}
    var full = data.full || {{}};
    ...
    var order = {{
      ...
      Assessment: ['verdict','purpose','evaluation'],
  ...
  function markersLegend() {{
    var s = '<b>Markers</b>'
      + '<span class="lg"><span class="sw" style="background:#2563eb;border:3px dashed #16a34a"></span>&ge;2 assessments</span>'
  ...
  function claimLegend() {{
    return '<b>Nodes (' + c.nodes_total + ')</b>'
      + lgNode('#2563eb','round-rectangle','Assertion + its folded assessment (' + (c.nodes_by_type.Assertion||0) + ')')
      ...
      + '<b>Verdict (assertion border)</b>'
      + lgBorder('#16a34a','supported_for_purpose (' + (v['supported_for_purpose']||0) + ')')
      + lgBorder('#d97706','insufficient (' + (v['insufficient']||0) + ')')
      + lgBorder('#dc2626','falsified (' + (v['falsified']||0) + ')')
      ...
      + lgLine('#4338ca','dashed','has_alternative — a rival reading, named')
      + lgLine('#a3a3a3','dotted','cites — prose citation, never evidence (dimmed)')
      + markersLegend()
      + memberLegend()
      + '<div class="hint" style="margin-top:6px">'
        + (c.folded_assessments||0) + ' assessment(s) and ' + (c.folded_grounds||0)
        ...
        + ((DATA.meta.navigable_nodes||0) > 0
            ...
            : '')
        + 'Use <i>Full graph</i> to see the Assessments and Grounds uncollapsed.</div>';
  }}
  function fullLegend() {{
    ...
    return '<b>Nodes — every Object as authored (' + cy.nodes().length + ')</b>'
      ...
      + lgNode('#7c3aed','diamond','Assessment — an authored verdict (' + (tc.Assessment||0) + ')')
      ...
      + lgLine('#7c3aed','solid','assessed_by — assertion &rarr; assessment')
      ...
      + lgLine('#4338ca','dashed','has_alternative')
      + lgLine('#a3a3a3','dotted','cites')
      + markersLegend()
      + memberLegend()
      + '<div class="hint" style="margin-top:6px">Nothing is folded. v6 hides much less than v2 did: '
        + 'there is no Bearing layer, and a Ground sits directly on the assertion it bears on. '
        + 'Use <i>Claim map</i> to fold the assessments and Grounds back in.</div>';
  }}
...
(function () {{
  var DATA = JSON.parse(document.getElementById('graph-data').textContent);
  // v6 cross-artifact reference kinds. In v6 a relationship is always INTERNAL to an
  ...
  function renderEdge(d){{
    detail.innerHTML='<h2>'+esc(d.rel)+'</h2><dl><dt>from</dt><dd>'+esc(d.source)+'</dd>'
      ...
      +'<p class="hint">In v6 every relationship is internal to one artifact, so this edge is not a relationship — it is derived from an <b>address</b> held in a property value.</p>';
  }}
...
#: Colours for the DERIVED cross-artifact edges on the overview. v6 stores no
...
# the one link form (CANONICAL-v6 3.1). Escape-first, so no authored markup ever
