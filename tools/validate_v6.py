"""Conformance validator for Symposium specification v6, hackathon profile.
...
submitting; the admin gate runs the identical code before accepting. Same verdict both times.
  ...
  python validate_v6.py <candidate.json> [record_dir/]
...
Conforms to spec/symposium_specification_v6.md and tools/CANONICAL-v6.md.
"""
...
ARTIFACT_TYPES = {"Argument", "Data", "ScientificPublication", "Analysis", "Model", "Report", "Message"}
NON_GROUNDABLE_TYPES = {"Analysis", "Report", "Message"}                   # spec 2.1
# Model is groundable (spec 2.6): whether its content is genuinely evidential is a judgment
# for the Ground's rationale and the Assessment, not a type-level prohibition.
ARGUMENT_OBJECTS = {"Assertion", "Assessment", "Ground", "Assumption", "AddressingMethod"}
RELS = {"depends_on", "has_alternative", "assessed_by", "grounded_by", "assumes"}
REL_TARGET_TYPE = {"depends_on": "Assertion", "has_alternative": "Assertion",
                   "assessed_by": "Assessment", "grounded_by": "Ground", "assumes": "Assumption"}
...
def build_index(artifacts):
    ...
    for a in artifacts:
        ...
        idx[h.get("name")] = {
            ...
            "objects": objs, "art": a,
            "methods": {n: o for n, o in objs.items() if o.get("type") == "AddressingMethod"},
        }
    return idx
...
def resolve(addr, index, members):
    ...
    if method:
        if method not in rec["methods"]:
            return False, None, f"'{root}' declares no AddressingMethod named '{method}'"
        info["method_obj"] = rec["methods"][method]
    return True, info, ""


def verify_content(info):
    """Machine-verify text_span / csv references against embedded content. -> [findings]"""
...
def groundable(info, citing_artifact):
    ...
    if info.get("node_type") == "AddressingMethod":
        return False, "FAIL", "AddressingMethod Objects are not themselves groundable (spec 2.2.4)"
    if info.get("node_type") == "Assertion":
        return True, None, ""                                   # an Assertion in another Argument
    m = info.get("method_obj")
    if m is None:
        return False, "FAIL", ("names no AddressingMethod; groundable content must be reached by a "
                               "method the Artifact declares (spec 2.2.4)")
    if m.get("groundable") is not True:
        return False, "FAIL", f"AddressingMethod '{info['method']}' is not declared groundable"
...
def check_structure(a):
    ...
    for r in a.get("relationships", []):
        if r.get("rel") not in RELS:
            f.append(finding("STRUCT", "FAIL", f"unknown relationship '{r.get('rel')}'"))
...
def check_type_specific(a):
    ...
    req = {"Analysis": ("procedure",), "Model": ("modeling_choices",),
           "Report": ("text",), "Message": ("recipients", "text")}.get(t, ())
    for k in req:
        if not h.get(k) and h.get(k) != []:
            f.append(finding("TYPE", "FAIL", f"{t} requires '{k}'"))
    if t == "Analysis" and "outputs" not in h:
        f.append(finding("TYPE", "FAIL", "Analysis requires 'outputs' (may be empty; spec 2.5)"))
    ...
    if t == "Argument" and not h.get("authors"):
        f.append(finding("TYPE", "FAIL", "Argument requires 'authors' (spec 2.2)"))
    ...
    for o in a.get("objects", []):
        ...
        if t != "Argument" and ot != "AddressingMethod":
            f.append(finding("TYPE", "FAIL", f"a {t} Artifact may contain only AddressingMethod Objects"))
        need = {"Assertion": ("claim", "scope"), "Assessment": ("verdict", "evaluation"),
                "Ground": ("address", "rationale"), "Assumption": ("rationale",),
                "AddressingMethod": ("description",)}.get(ot, ())
        for k in need:
            if not str(o.get(k, "")).strip():
                f.append(finding("TYPE", "FAIL", f"{ot} '{o.get('name')}' missing '{k}'"))
        if ot == "Assessment" and o.get("verdict") not in VERDICTS:
            f.append(finding("TYPE", "FAIL",
                             f"Assessment '{o.get('name')}' verdict {o.get('verdict')!r} not in {sorted(VERDICTS)}"))
        if ot == "AddressingMethod":
            if not isinstance(o.get("groundable"), bool):
                f.append(finding("TYPE", "FAIL",
                                 f"AddressingMethod '{o.get('name')}' groundable must be a boolean, "
                                 f"got {o.get('groundable')!r} (spec 1.7.1)"))
            if o.get("name") not in STD_METHODS:
                f.append(finding("TYPE", "REVIEW",
                                 f"AddressingMethod '{o.get('name')}' is outside the profile's standard set"))
        ...
        if ot == "AddressingMethod" and h.get("type") in NON_GROUNDABLE_TYPES and o.get("groundable") is True:
            f.append(finding("TYPE", "FAIL",
...
def check_argument(a):
    ...
    S = {n for n, o in objs.items() if o["type"] == "Assessment"}
    ...
    # 1:1 Assertion <-> Assessment
    by_src, by_tgt = {}, {}
    for r in rels:
        if r.get("rel") == "assessed_by":
            by_src.setdefault(r["source"], []).append(r["target"])
            by_tgt.setdefault(r["target"], []).append(r["source"])
    for n in sorted(A):
        k = len(by_src.get(n, []))
        if k != 1:
            f.append(finding("C-ARG", "FAIL", f"Assertion '{n}' has {k} Assessments (must be exactly 1)"))
    for n in sorted(S):
        k = len(by_tgt.get(n, []))
        if k != 1:
            f.append(finding("C-ARG", "FAIL", f"Assessment '{n}' belongs to {k} Assertions (must be exactly 1)"))
    ...
    alts = {r["target"] for r in rels if r.get("rel") == "has_alternative"}
    for n in sorted(alts & depended_on):
        f.append(finding("C-ARG", "FAIL", f"alternative Assertion '{n}' must be a root of the depends_on DAG"))
    for r in rels:
        if r.get("rel") == "has_alternative" and r.get("source") != pa:
            f.append(finding("C-ARG", "REVIEW",
                             f"has_alternative from non-primary Assertion '{r.get('source')}' (spec 2.2 scopes it to the primary)"))

    # purpose required on the primary Assessment
    for tgt in by_src.get(pa, []):
        if not str(objs.get(tgt, {}).get("purpose", "")).strip():
            f.append(finding("C-ARG", "FAIL", f"the primary Assertion's Assessment '{tgt}' requires a purpose"))
    return f
...
def check_independence(a, index):
    ...
    and belongs in the Assessment's `evaluation` (spec 2.2.4).
...
def check_corpus(a, index, members, record_names):
    ...
    for n, o in objs.items():
        if o["type"] == "Ground":
            check_addr(o.get("address", ""), f"Ground '{n}'", True)

    # prose citations: markdown link form only (profile 3.1)
    for scope, d in [("header", h)] + [(f"Object '{n}'", o) for n, o in objs.items()]:
        for k, v in d.items():
            if not isinstance(v, str) or k in ("address", "published_by", "produced_by", "extracted_from"):
                continue
